...
def format_message(id:int, message: str) -> Transaction | None:
    try:
        if message.startswith('BUY') or message.startswith('SELL'):
            ...
            return Transaction(id_=id, Order=Order, Where=Where, Emmet=Emmet, CMP=float(CMP), TP=float(TP), SL=float(SL), Risk=float(Risk), Update='')
            ...
            return Transaction(id_=id, Order=Order, Where=f'{Change_line[1]}', Emmet=Emmet, CMP=float(CMP), TP=float(TP), SL=float(SL), Risk=float(0), Update=Change_line[-1])
        elif message.startswith('Close'):
            Order, Where, Emmet, Other = message.split(' ', 3)
            CMP = Other.split()[2][:-1]
            return Transaction(id_=id, Order=Order, Where=Where, Emmet=Emmet, CMP=float(CMP), TP=float(0), SL=float(0), Risk=float(0), Update='')
    except Exception:
        return None
